#!/bin/bash
#
# Script file to stop all docker machines.
#
...
Usage () {
	echo
	echo "Description:"
	echo "Script file to stop all docker machines."
	echo
...
is_proceed=$(Prompt "Confirm to stop docker machines ${machines[@]}?")
if [ "$is_proceed" != "Y" ]; then
	echo "Abort stopping docker machine."
	exit
fi
